...
def plot_measurements(xs, ys=None, dt=None, color='k', lw=1, label='Measurements',
    ...
    if lines:
        if ys is not None:
            return plt.plot(xs, ys, color=color, lw=lw, ls='--', linewidth=0.1,label=label, **kwargs)
        else:
            return plt.plot(xs, color=color, lw=lw, ls='--', label=label, **kwargs)
    else:
        if ys is not None:
            return plt.scatter(xs, ys, edgecolor=color, facecolor='none', linewidth=0.1,
                        lw=2, label=label, **kwargs),
        else:
            return plt.scatter(range(len(xs)), xs, edgecolor=color, facecolor='none', linewidth=0.1,
                        lw=2, label=label, **kwargs),
